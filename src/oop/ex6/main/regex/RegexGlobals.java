...
public class RegexGlobals {
    ...
    //int
    public final static String INT_FINAL_PARSER = "\\s*(final\\s+int)\\s+((" + NAME_FORMAT + "\\s*=\\s*"
            + INT_FORMAT + ")(\\s*,\\s*" + NAME_FORMAT + "\\s*=\\s*" + INT_FORMAT + ")*)\\s*;\\s*";
    public final static String INT_DECLARATION_INIT_PARSER = "\\s*(int)\\s+(" + NAME_FORMAT
            + "(\\s*=\\s*" + INT_FORMAT + ")?\\s*(\\s*,\\s*" + NAME_FORMAT + "(\\s*=\\s*" +
            INT_FORMAT + ")?)*)\\s*;\\s*";
    //double
    public final static String DOUBLE_FINAL_PARSER = "\\s*(final\\s+double)\\s+((" + NAME_FORMAT
            + "\\s*=\\s*" + DOUBLE_FORMAT + ")\\s*(\\s*,\\s*" + NAME_FORMAT + "\\s*=\\s*" +
            DOUBLE_FORMAT + ")*)\\s*;\\s*";
    public final static String DOUBLE_DECLARATION_INIT_PARSER = "\\s*(double)\\s+(" + NAME_FORMAT +
            "(\\s*=\\s*" + DOUBLE_FORMAT + ")?\\s*(\\s*,\\s*" + NAME_FORMAT + "(\\s*=\\s*" +
            DOUBLE_FORMAT + ")?)*)\\s*;\\s*";
    //string
    public final static String STRING_FINAL_PARSER = "\\s*(final\\s+String)\\s+((" + NAME_FORMAT +
            "\\s*=\\s*" + STRING_FORMAT + ")(\\s*,\\s*" + NAME_FORMAT + "\\s*=\\s*" + STRING_FORMAT +
            ")*)\\s*;\\s*";
    public final static String STRING_DECLARATION_INIT_PARSER = "\\s*(String)\\s+(" + NAME_FORMAT +
            "(\\s*=\\s*" + STRING_FORMAT + ")?\\s*(\\s*,\\s*" + NAME_FORMAT + "(\\s*=\\s*" +
            STRING_FORMAT + ")?)*)\\s*;\\s*";
    //boolean
    public final static String BOOLEAN_FINAL_PARSER = "\\s*(final\\s+boolean)\\s+((" + NAME_FORMAT +
            "\\s*=\\s*" + BOOLEAN_FORMAT + ")(\\s*,\\s*" + NAME_FORMAT + "\\s*=\\s*" + BOOLEAN_FORMAT +
            ")*)\\s*;\\s*";
    public final static String BOOLEAN_DECLARATION_INIT_PARSER = "\\s*(boolean)\\s+(" + NAME_FORMAT +
            "(\\s*=\\s*" + BOOLEAN_FORMAT + ")?\\s*(\\s*,\\s*" + NAME_FORMAT + "(\\s*=\\s*" +
            BOOLEAN_FORMAT + ")?)*)\\s*;\\s*";
    //char
    public final static String CHAR_FINAL_PARSER = "\\s*(final\\s+char)\\s+((" + NAME_FORMAT +
            "\\s*=\\s*" + CHAR_FORMAT + ")(\\s*,\\s*" + NAME_FORMAT + "\\s*=\\s*" +
            CHAR_FORMAT + ")*)\\s*;\\s*";
    public final static String CHAR_DECLARATION_INIT_PARSER = "\\s*(char)\\s+(" + NAME_FORMAT +
            "(\\s*=\\s*" + CHAR_FORMAT + ")?\\s*(\\s*,\\s*" + NAME_FORMAT + "(\\s*=\\s*" +
            CHAR_FORMAT + ")?)*)\\s*;\\s*";

    public final static String ASSIGNMENT = "\\s*(" + VARIABLE_NAME_REGEX + "\\s*=\\s*" +
            ALL_FORMAT + ")\\s*(\\s*," + "\\s*" + VARIABLE_NAME_REGEX + "\\s*=\\s*" +
            ALL_FORMAT + "\\s*)*;\\s*";
